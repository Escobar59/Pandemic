...
def bordingBoxCalculator(person):
    '''
    Will create a table where is stock the coordonate of every bording box
    '''
    tab = []
    for i in person:
        tab.append( (i[0] - constants.PERSON_RADIUS, 
                     i[1] - constants.PERSON_RADIUS,
                     i[0] + constants.PERSON_RADIUS, 
                     i[1] + constants.PERSON_RADIUS)
                     )
        
    return tab

def circleCollision(c1, c2, borderBoxListe):
    '''
    Determines if two circles intersect or not
    '''
    # Calculate bounding boxes for the circles
    #c1_bbox = (c1[0] - constants.PERSON_RADIUS, c1[1] - constants.PERSON_RADIUS,
    #           c1[0] + constants.PERSON_RADIUS, c1[1] + constants.PERSON_RADIUS)

    #c2_bbox = (c2[0] - constants.PERSON_RADIUS, c2[1] - constants.PERSON_RADIUS,
    #           c2[0] + constants.PERSON_RADIUS, c2[1] + constants.PERSON_RADIUS)

    # Check if bounding boxes intersect
    if not (borderBoxListe[c1][2] < borderBoxListe[c2][0] or
            borderBoxListe[c1][0] > borderBoxListe[c2][2] or
            borderBoxListe[c1][3] < borderBoxListe[c2][1] or
            borderBoxListe[c1][1] > borderBoxListe[c2][3]):
        ...
        # # Calculate squared distance between c1 and c2
        # dist_squared = vector[0] ** 2 + vector[1] ** 2

        # # Calculate the sum of radii squared
        # sum_radii_squared = (2 * constants.PERSON_RADIUS) ** 2

        # # Check for collision using squared distance and sum of radii squared
        # if dist_squared < sum_radii_squared:
        #     return True
...
def computeCollisions(persons, personNumber, borderBoxListe):
    '''
    ...
    for j in range (len(persons)):
        ...
        if ( j != personNumber):
            ...
            if circleCollision( personNumber, j, borderBoxListe):
